@extends('layouts.app', ['title' => 'Surat Perintah Jalan Manual'])
...
@php
  ...
  $pickupTime = $s->pickup_time ? \Carbon\Carbon::parse($s->pickup_time)->format('H:i') : '';
  $paymentPlan = match($s->payment_plan) {
    ...
    default => ucfirst($s->payment_plan ?? '-'),
  };
  $tripHtml = $s->trip_details ?? '';
  $tripText = trim(strip_tags(str_replace(['<br>','<br/>','<br />'], "\n", $tripHtml)));
  $tripLines = array_values(array_filter(array_map(fn($l) => trim($l), preg_split("/\r\n|\n|\r/", $tripText))));
@endphp
<style>
  .spj-wrap {
    position: relative;
...
<div class="spj-wrap">
  ...
  <div class="section">
    <div class="grid-2">
      <div class="label">Nama Supir</div><div>{{ $s->driver_name }}</div>
      <div class="label">Nama Pelanggan</div><div>{{ $s->customer_name }}</div>
      <div class="label">Nomor Kontak</div><div>{{ $s->customer_contact ?: '-' }}</div>
      <div class="label">Negara Asal</div><div>{{ $s->country_of_origin ?: '-' }}</div>
      <div class="label">Jumlah Penumpang</div><div>{{ $s->passenger_count }}</div>
      <div class="label">Tanggal Mulai</div><div>{{ $start }}</div>
      <div class="label">Waktu Penjemputan</div><div>{{ $pickupTime }}</div>
      <div class="label">Alamat Penjemputan</div><div>{{ $s->pickup_address }}</div>
      <div class="label">Layanan</div><div>{{ $s->service_type ?: '-' }}</div>
      <div class="label">Durasi Layanan</div><div>{{ $s->service_duration ?: '-' }}</div>
      <div class="label">Rencana Pembayaran</div><div>{{ $paymentPlan }}</div>
      <div class="label">Rincian Perjalanan</div>
      <div>
        @if(trim($tripHtml) !== '')
          <div class="wysiwyg-view">
            <div class="w-content">{!! nl2br(e($s->trip_details)) !!}</div>
          </div>
        @else
          -
        @endif
