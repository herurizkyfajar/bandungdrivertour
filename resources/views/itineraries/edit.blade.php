@extends('layouts.app', ['title' => 'Edit Itinerary'])
...
<div class="dashboard-wrap">
  ...
  <div class="content-card">
    ...
    <form method="POST" action="{{ route('itineraries.update', $itinerary) }}" id="itineraryForm">
      @csrf
      @method('PUT')
      <div style="margin-bottom:1rem; display:flex; align-items:center; justify-content:space-between;">
        <div>
          <h1>Edit Itinerary</h1>
          <div class="subtitle">Update travel itinerary.</div>
        </div>
      ...
      <div class="form-grid">
        ...
        <div class="col-6">
          <div class="field">
            ...
            <div class="search-select" id="user_select">
              <input id="user_search" class="search-input" type="text" placeholder="Search user name..." autocomplete="off" value="{{ old('user_search', $itinerary->user->name) }}">
              <input type="hidden" name="user_id" id="user_id_hidden" value="{{ old('user_id', $itinerary->user_id) }}">
          ...
          <div class="field">
            ...
            <select id="status" name="status" required>
              @foreach(['draft', 'active', 'done'] as $status)
                <option value="{{ $status }}" {{ old('status', $itinerary->status) === $status ? 'selected' : '' }}>{{ $status }}</option>
              @endforeach
            </select>
          </div>
        </div>
        <div class="col-12">
          <div class="field">
            ...
            <input id="title" type="text" name="title" value="{{ old('title', $itinerary->title) }}" required>
          </div>
        </div>
        <div class="col-12">
          <div class="field">
            <label for="description">Description</label>
            <textarea id="description" name="description" style="width:100%; border:1px solid #cbd5e1; border-radius:12px; padding:.75rem .9rem;">{{ old('description', $itinerary->description) }}</textarea>
          </div>
        </div>
        <div class="col-6">
          <div class="field">
            <label for="start_date">Start Date</label>
            <input id="start_date" type="date" name="start_date" value="{{ old('start_date', $itinerary->start_date->format('Y-m-d')) }}" required>
          </div>
        </div>
        <div class="col-6">
          <div class="field">
            <label for="end_date">End Date</label>
            <input id="end_date" type="date" name="end_date" value="{{ old('end_date', $itinerary->end_date->format('Y-m-d')) }}" required>
          </div>
        </div>
      </div>
  ...
  @foreach($itinerary->days as $day)
    {
      ...
      activities: [
        @foreach($day->activities as $act)
          { time_from: "{{ substr($act->time_from, 0, 5) }}", time_to: "{{ substr($act->time_to, 0, 5) }}", activity: @json($act->activity) },
        @endforeach
      ]
    },
  @endforeach
...
function addDayWithData(dayNum, dateStr, activities) {
  var container = document.getElementById('daysContainer');
  var html = '<div class="day-card" data-day-index="' + dayIndex + '">'
    + '<div class="day-header">'
    + '<h3>Day ' + dayNum + ' <input type="hidden" name="days[' + dayIndex + '][day_number]" value="' + dayNum + '"><input type="hidden" name="days[' + dayIndex + '][date]" value="' + dateStr + '"></h3>'
  ...
  var actContainer = container.querySelector('.activities-container[data-day-idx="' + dayIndex + '"]');
  activities.forEach(function(act) {
    addActivityWithData(actContainer, dayIndex, act);
  });

  dayIndex++;
}

function addActivityWithData(container, di, act) {
  var actIdx = container.querySelectorAll('.activity-row').length;
  var html = '<div class="activity-row">'
    ...
    + '</div>'
    + '<input type="time" class="act-time-from" name="days[' + di + '][activities][' + actIdx + '][time_from]" value="' + (act.time_from || '') + '" required>'
    + '<input type="time" class="act-time-to" name="days[' + di + '][activities][' + actIdx + '][time_to]" value="' + (act.time_to || '') + '" required>'
    + '<input type="text" class="act-activity" name="days[' + di + '][activities][' + actIdx + '][activity]" placeholder="Activity..." value="' + (act.activity || '') + '" required>'
...
function addDay() {
  ...
  addDayWithData(currentDayNum, dateStr, [{ time_from: '', time_to: '', activity: '' }]);
}
...
function addActivity(btn, di) {
  ...
  addActivityWithData(container, di, { time_from: '', time_to: '', activity: '' });
}
...
if (EXISTING_DAYS.length) {
  EXISTING_DAYS.forEach(function(day) {
    addDayWithData(day.day_number, day.date, day.activities.length ? day.activities : [{ time_from: '', time_to: '', activity: '' }]);
  });
} else {
  addDay();
}
